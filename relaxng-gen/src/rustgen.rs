...
impl Context {
    ...
    fn get_config_from_last_ref(&self) -> ConfigRule {
    ...
    fn pop_state(&mut self) {
        ...
        let config = self.get_config_from_last_ref();
    ...
    fn value(&mut self, val: &str) {
        ...
        if let Some(name) = name {
            let xpath_str = self.xpath_from_last_ref();
            debug!(?name, ?xpath_str);
        }
        self.add_field(val, name, FieldTy::Value(val.to_string()));
        self.pop_state();
    }
