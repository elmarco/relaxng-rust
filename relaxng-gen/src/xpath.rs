...
fn parse_index_predicate(input: &str) -> IResult<&str, XPathPredicate> {
    map(
        delimited(char('['), nom::character::complete::u64, char(']')),
        |index| XPathPredicate::Index(index as usize),
    )
    .parse(input)
}
...
fn parse_expr_with_predicate(input: &str) -> IResult<&str, (XPathExpr, Option<XPathExpr>)> {
    map((parse_node, opt(parse_predicate)), |(node, predicate)| {
        (node, predicate.map(XPathExpr::Predicate))
    })
    .parse(input)
}
